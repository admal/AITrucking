...
CONST_DLL_VJOY = "C:\\Users\\Adam\\Documents\\NN\\AITruckingProject\\AITrucking\\Controls\\vJoyInterface.dll"
...
def steer(steering_angle):
    #axis_val = int((steering_angle + 1) * 16000)
    axis_val = 16000 - int((steering_angle * 10000))
    joystickPosition = vj.generateJoystickPosition(wAxisX = axis_val)
    vj.update(joystickPosition)
...
def throttle(throttle_val = 32786):
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisZRot = throttle_val)
    vj.update(joystickPosition)
    vj.close()


def reverse_brake(brake_val = 32786):
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisZ = brake_val)
    vj.update(joystickPosition)
    vj.close()
